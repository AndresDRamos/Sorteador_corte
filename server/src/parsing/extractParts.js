// Separa BLOCKs reales (piezas) de los blocks-datum, y agrupa INSERTs por numero de parte.
...
// Extrae piezas y datums del bloque BLOCKS, e instancias del bloque ENTITIES.
export function extractParts(parsed) {
  const blocks = parsed.blocks || {};
  // partDefinitions agrupa la geometria de cada pieza unica indexada por numero de parte.
  const partDefinitions = new Map();
  // datumDefinitions almacena la geometria de los blocks-datum para renderizarlos en gris.
  const datumDefinitions = new Map();

  for (const name of Object.keys(blocks)) {
    const block = blocks[name];
    const entities = normalizeBlockEntities(block.entities);
    if (isDatumName(name)) {
      datumDefinitions.set(name, { entities });
    } else {
      // Solo registramos blocks que aporten geometria renderizable.
  ...
  for (const e of allEntities) {
    ...
    if (isDatumName(partNumber)) {
      datumInstances.push(inst);
    } else if (partDefinitions.has(partNumber)) {
      instances.push(inst);
      countByPart.set(partNumber, (countByPart.get(partNumber) || 0) + 1);
    }
    // Si referencia un block desconocido, lo ignoramos silenciosamente en v1.
  }

  return { partDefinitions, datumDefinitions, instances, datumInstances, countByPart };
}
